...
final class YoOhw_COS_Customer_Query {
	...
	public static function sanitize_args( array $source ): array {
		...
		return array(
			...
			'rfm_recency_max_days' => self::sanitize_rfm_integer( $source['rfm_recency_max_days'] ?? '', 3650 ),
			'rfm_frequency_min' => self::sanitize_rfm_integer( $source['rfm_frequency_min'] ?? '', 100000 ),
			'rfm_monetary_min' => self::sanitize_rfm_money( $source['rfm_monetary_min'] ?? '' ),
