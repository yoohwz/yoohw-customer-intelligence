...
final class YoOhw_COS_Integration_Smoke_Test extends WP_UnitTestCase {
	...
	public function test_current_schema_recovers_missing_currency_backfill_and_old_issue_ledger(): void {
		global $wpdb;
		$previous_migrations = get_option( 'yoohw_cos_data_migrations', array() );
		YoOhw_COS_Customers::reset_data();
		$single = $this->create_order( 'missing-backfill-single@example.test', 'completed', '45.00' );
		...
		$wpdb->insert( YoOhw_COS_DB::migration_issues_table(), array(
			'migration_id' => 'commerce_facts_v2', 'object_type' => 'order', 'object_id' => $single->get_id(),
		...
		try {
			...
			$this->assertFalse( YoOhw_COS_Commerce_Metrics_Policy::money_is_comparable( YoOhw_COS_Customers::get_customer( $single_id ) ) );
			for ( $batch = 0; $batch < 20; ++$batch ) {
				YoOhw_COS_Migration_Runner::run_next_batch();
				if ( 'completed' === ( YoOhw_COS_Migration_Runner::get_state()['commerce_currency_v3']['status'] ?? '' ) ) { break; }
			}
			$state = YoOhw_COS_Migration_Runner::get_state();
			...
			YoOhw_COS_Install::maybe_update();
			$this->assertSame( $state['commerce_currency_v3']['attempts'], YoOhw_COS_Migration_Runner::get_state()['commerce_currency_v3']['attempts'] );
		} finally {
			update_option( 'yoohw_cos_data_migrations', $previous_migrations, false );
			wp_clear_scheduled_hook( YoOhw_COS_Migration_Runner::HOOK );
		}
	...
	public function test_current_facts_still_retry_an_old_unattributable_issue(): void {
		global $wpdb;
		$previous_migrations = get_option( 'yoohw_cos_data_migrations', array() );
		YoOhw_COS_Customers::reset_data();
		$order = $this->create_order( 'current-facts-old-issue@example.test', 'completed', '25.00' );
		$order->set_billing_phone( '+1 415 555 8671' );
		$order->save();
		$customer_id = YoOhw_COS_Customers::sync_from_order( $order );
		$unattributable = wc_create_order();
		$unattributable->set_status( 'completed' );
		...
		try {
			$this->assertSame( YoOhw_COS_Commerce_Metrics_Policy::VERSION, (int) $wpdb->get_var( $wpdb->prepare( 'SELECT policy_version FROM %i WHERE order_id = %d', YoOhw_COS_DB::order_facts_table(), $order->get_id() ) ) );
			$this->assertSame( '0', (string) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM %i WHERE policy_version < %d OR currency IS NULL OR currency = ''", YoOhw_COS_DB::order_facts_table(), YoOhw_COS_Commerce_Metrics_Policy::VERSION ) ) );
			YoOhw_COS_Install::maybe_update();
			$this->assertSame( 'pending', YoOhw_COS_Migration_Runner::get_state()['commerce_currency_v3']['status'] );
			for ( $batch = 0; $batch < 20; ++$batch ) {
				YoOhw_COS_Migration_Runner::run_next_batch();
			...
			$this->assertSame( 'completed', YoOhw_COS_Migration_Runner::get_state()['commerce_facts_v2']['status'] );
			$this->assertSame( 'resolved', $wpdb->get_var( $wpdb->prepare( 'SELECT status FROM %i WHERE migration_id = %s AND object_id = %d', YoOhw_COS_DB::migration_issues_table(), 'commerce_facts_v2', $unattributable->get_id() ) ) );
			$this->assertSame( 'comparable', YoOhw_COS_Customers::get_customer( $customer_id )['money_state'] );
	...
	public function test_completed_backfill_hides_stale_decisions_after_interrupted_order_refresh(): void {
		global $wpdb;
		$previous_migrations = get_option( 'yoohw_cos_data_migrations', array() );
		update_option( 'yoohw_cos_data_migrations', array( 'commerce_currency_v3' => array( 'status' => 'completed' ) ), false );
		try {
			$currency = get_woocommerce_currency();
			$other_currency = 'USD' === $currency ? 'EUR' : 'USD';
			$first = $this->create_order( 'completed-refresh@example.test', 'completed', '3000.00' );
			$second = $this->create_order( 'completed-refresh@example.test', 'completed', '3000.00' );
			$id = YoOhw_COS_Customers::sync_from_order( $first );
			YoOhw_COS_Customers::sync_from_order( $second );
			...
			$this->assertSame( 1, YoOhw_COS_Customer_Query::query( array( 'vip_status' => 'high_value' ) )['total_items'] );
			...
			$this->assertSame( '0', (string) $wpdb->get_var( $wpdb->prepare( 'SELECT intelligence_currency_ready FROM %i WHERE id = %d', YoOhw_COS_DB::customers_table(), $id ) ) );
			$this->assertSame( 'mixed', YoOhw_COS_Customers::get_customer( $id )['money_state'] );
			...
			$this->assertGreaterThan( 0, YoOhw_COS_Customers::sync_from_order( $second ) );
	...
	public function test_store_currency_change_gates_persisted_decisions_until_refresh_completes(): void {
		$previous_migrations = get_option( 'yoohw_cos_data_migrations', array() );
		update_option( 'yoohw_cos_data_migrations', array( 'commerce_currency_v3' => array( 'status' => 'completed' ) ), false );
		$currency = get_woocommerce_currency();
		$other_currency = 'USD' === $currency ? 'EUR' : 'USD';
		try {
			$first = $this->create_order( 'store-currency-change@example.test', 'completed', '3000.00' );
			$second = $this->create_order( 'store-currency-change@example.test', 'completed', '3000.00' );
			$id = YoOhw_COS_Customers::sync_from_order( $first );
			YoOhw_COS_Customers::sync_from_order( $second );
			YoOhw_COS_Customers::process_risk_score_cache_refresh( -1 );
			$this->assertSame( 'platinum', YoOhw_COS_Customers::get_customer( $id )['vip_status'] );
	...
	private function load_plugin_classes(): void {
		...
		$files = array(
			...
			'includes/class-yoohw-cos-extensions.php',
			'includes/class-yoohw-cos-customer-facts.php',
			'includes/class-yoohw-cos-attention.php',
