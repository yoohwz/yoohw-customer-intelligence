...
final class YCI_Privacy_Erasure_Test extends WP_UnitTestCase {
	...
	public function test_reset_between_pages_keeps_bounded_order_link_snapshot_for_completion(): void {
		global $wpdb;
		$order = wc_create_order();
		...
		$snapshot = (string) $wpdb->get_var( $wpdb->prepare( 'SELECT option_value FROM %i WHERE option_name LIKE %s LIMIT 1', $wpdb->options, $wpdb->esc_like( 'yoohw_cos_privacy_erasure_links_' ) . '%' ) );
		$this->assertNotEmpty( $snapshot );
		$this->assertStringNotContainsString( 'reset-linked@example.test', $snapshot );
		...
		for ( $page = 2; $page <= 5; $page++ ) {
			$result = YoOhw_COS_Privacy_Erasure::erase( 'reset-linked@example.test', $page );
		...
		$this->assertSame( '', (string) $fresh->get_meta( YoOhw_COS_Reset_Guard::META_KEY ) );
		$this->assertSame( 'keep', $fresh->get_meta( '_other_plugin' ) );
		$this->assertSame( 'reset-linked@example.test', $fresh->get_billing_email() );
		...
		$this->assertSame( 0, YoOhw_COS_Customers::sync_from_order_id( $order->get_id() ) );
		$this->assertFalse( wp_next_scheduled( YoOhw_COS_Customers::ORDER_SYNC_RETRY_HOOK, array( $order->get_id() ) ) );
		$this->assertSame( '0', (string) $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM %i WHERE option_name LIKE %s', $wpdb->options, $wpdb->esc_like( 'yoohw_cos_privacy_erasure_links_' ) . '%' ) ) );
	}
