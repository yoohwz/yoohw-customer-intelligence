...
| Fresh staged install and schema | PASS | Disposable WP 6.9/WC 10.8.0 installation activated the staged tree; DB version `0.2.4` and 12 Customer Intelligence tables were observed. |
| Existing 1.3.0 overlay: bounded persisted fixture | PASS | A disposable tagged `1.3.0` fixture retained one customer, note, task, tag and segment after overlay; DB version converged `0.2.1` to `0.2.4`. |
| Broader existing-install order, Saved View, reset, privacy and suppression persistence | NOT_RUN | The direct overlay fixture did not cover these persisted states. The two-mode integration suite covers current-tree behavior, but is not an upgrade survival proof. |
...
A separate disposable `/tmp/yci62.*` WordPress installation used a newly initialized socket-only MySQL 8.4.0 data directory, WP 6.9 and WC 10.8.0. The trusted staged product was copied into its plugin directory. `wp plugin activate yoohw-customer-intelligence` succeeded without a fatal error, `wp option get yoohw_cos_db_version` returned `0.2.4`, and `SHOW TABLES LIKE 'wp_yoohw_cos_%'` returned 12 tables: customers, events, notes, tasks, tags, customer_tags, segments, customer_segments, customer_order_facts, notification_log, migration_issues and privacy_suppression.

The same disposable site then activated repository tag `1.3.0` (`db_version=0.2.1`) and used plugin APIs to create one synthetic customer, note, task, tag and segment. After deactivation, replacing its files with the staged development tree and reactivation, `db_version` was `0.2.4`; the synthetic customer retained its email/display name, and notes, tasks, tags and segments each still had one row. These observations are a bounded forward-overlay check. They do not prove order-fact convergence, relationship membership, Saved View persistence, reset ownership, or privacy/suppression upgrade behavior. The runner's HPOS=yes/no tests verify both current storage paths, but the overlay fixture did not repeat the historical-data transition in both modes. WooCommerce-inactive handling was not directly exercised in this manual fixture. These gaps remain unverified, not inferred from successful activation.
...
- **Unverified audit portions:** direct upgrade survival for orders/links, Saved Views, reset/privacy/suppression and the admin visual checklist are `NOT_RUN`. A release candidate must not infer them from current-tree tests. Version-coupled 1.4.0 behavior is explicitly deferred.
