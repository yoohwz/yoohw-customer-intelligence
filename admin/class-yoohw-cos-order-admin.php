...
final class YoOhw_COS_Order_Admin {
	...
	public static function handle_customer_search(): void {
		...
		foreach ( self::search_customers( $term ) as $customer ) {
			$customer_id = absint( $customer['id'] ?? 0 );

			if ( $customer_id > 0 ) {
				$results[ $customer_id ] = self::format_customer_option( $customer );
			}
		}
	...
	public static function render_customer_field( WC_Order $order ): void {
		...
		$customer_help_tip = __( 'Uses Customer Intelligence profiles. WooCommerce customer user is synchronized when the selected profile has a WP user.', 'yoohw-customer-intelligence' );
	...
	private static function search_customers( string $term ): array {
		global $wpdb;
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, wp_user_id, email, phone, first_name, last_name, display_name FROM %i {$where} ORDER BY updated_at DESC LIMIT %d",
				...$params
			),
			ARRAY_A
		);
	...
	private static function format_customer_option( array $customer ): string {
		$customer_id  = absint( $customer['id'] ?? 0 );
